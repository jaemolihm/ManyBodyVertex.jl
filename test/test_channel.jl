...
@testset "channels" begin
    for C in (:A, :P, :T)
        inds_standard = mfRG.indices_to_standard(C, (1, 2, 3, 4))
        @test mfRG.indices_to_channel(C, inds_standard) == (1, 2, 3, 4)

        vvw = rand(3)
        v1234 = mfRG.frequency_to_standard(Val(:ZF), C, vvw...)
        @test all(mfRG.frequency_to_channel(Val(:ZF), C, v1234...) .≈ vvw)
    end

    for f in (:MF, :ZF, :KF)
        for C in (:A, :P, :T)
            for v1 in -2:2, v2 in -2:2, w in -2:2
                x = mfRG.frequency_to_standard(Val(f), C, v1, v2, w)
                y = mfRG.frequency_to_channel(Val(f), C, x...)
                @test sum(x) == (f === :MF ? -2 : 0)
                @test y == (v1, v2, w)
            end
        end
    ...
    for f in (:MF, :ZF, :KF), C in (:A, :P, :T)
        for (v, w) in [(10, 2), (-5, 2), (6, -3), (-5, -11)]
            v1, v2 = mfRG._bubble_frequencies(Val(f), C, v, w)
            @test all((v, w) .≈ mfRG._bubble_frequencies_inv(Val(f), C, v1, v2))
        end
        for (v1, v2) in [(10, 2), (-5, 2), (6, -3), (-5, -11)]
            v, w = mfRG._bubble_frequencies_inv(Val(f), C, v1, v2)
            @test all((v1, v2) .≈ mfRG._bubble_frequencies(Val(f), C, v, w))
        end
    end
    ...
    for C in (:A, :P, :T)
        ...
        inds_vertex = mfRG.indices_to_standard(C, (1, 2, 3, 4))
        inds_bubble = mfRG._bubble_indices(C, (3, 4, 1, 2))
        ...
        for f in (:MF, :ZF, :KF)
            for (vL, vB, vR, w) in ((1, 3, -6, 23), (0, -6, 4, 22))
                v1234_L = mfRG.frequency_to_standard(Val(f), C, vL, vB, w) .* (1, -1, 1, -1)
                v1234_R = mfRG.frequency_to_standard(Val(f), C, vB, vR, w) .* (1, -1, 1, -1)
                if f === :MF
                    # For imaginary frequencies, -v requires additional subtraction of 1.
                    v1234_L = v1234_L .- (0, 1, 0, 1)
                    v1234_R = v1234_R .- (0, 1, 0, 1)
                end
                v1, v2 = mfRG._bubble_frequencies(Val(f), C, vB, w)
    ...
    for C in (:A, :P, :T)
        v12 = vec([mfRG._bubble_frequencies.(Val(:MF), C, i, j) for i in -5:5, j in -5:5])
        @test allunique(v12)
