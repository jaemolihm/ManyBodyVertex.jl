...
@testset "RealSpaceVertex" begin
    using ManyBodyVertex: real_space_channel, real_space_convert_channel, apply_crossing
    ...
    @test ManyBodyVertex.get_formalism(x) == :MF
    ...
    y = ManyBodyVertex.cache_vertex_matrix(x, :P, [-1, 0, 1], (; freq=basis_v2, r=rbasis))
    @test real_space_channel(y) == :P
    @test get_channel(y) == :P
    @test y isa RealSpaceVertex{:MF}
    @test eltype(y.vertices_R) <: ManyBodyVertex.CachedVertex4P{:MF}
end
