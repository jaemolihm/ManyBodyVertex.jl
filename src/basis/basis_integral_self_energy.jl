"""
    basis_integral_self_energy(b1, b2, b3, v)
Basis integral for calculating self-energy: ``overlap = ∫dw b1(v + w/2) * b2(w) * b3(v + w)``
Can be used only for vertices in the A channel.
...
"""
function basis_integral_self_energy(b1::AbstractImagBasis, b2, b3, v)
    bases = (b1, b2, b3)
    sizes = nbasis.(bases)
    ...
    for inds in CartesianIndices(sizes)
        ...
        function f(w)
            v_w_over_2, _ = _bubble_frequencies_inv(Val(:MF), Val(:A), v, v + w)
...
function basis_integral_self_energy(b1::AbstractRealFreqBasis, b2, b3, v)
    bases = (b1, b2, b3)
    sizes = nbasis.(bases)
    overlap = zeros(eltype(bases[1]), sizes)
    for inds in CartesianIndices(sizes)
        ...
        function f(w)
            v_w_over_2, _ = _bubble_frequencies_inv(Val(:KF), Val(:A), v, v + w)
            b1[v_w_over_2, i1] * b2[w, i2] * b3[v+w, i3]
        end
        l, r = interval.left, interval.right
        (l >= r || l ≈ r) && continue
